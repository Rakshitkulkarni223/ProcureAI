...
exports.BasketOptimizationService = void 0;
...
class BasketOptimizationService {
    static buildPlan(items, consolidationPenalty = 0, weightProfile = 'balanced') {
        ...
        const analyses = items.map((item) => {
            ...
            for (const p of inStock) {
                ...
                if (!existing || p.price < existing.price)
                    bySupplier.set(p.provider, p);
            }
            let best = null;
            let reasons = [];
            if (bySupplier.size) {
                const rec = RecommendationService_1.RecommendationService.recommend([...bySupplier.values()], weightProfile);
                if (rec) {
                    best = rec.product;
        ...
        for (const s of allSuppliers) {
            ...
            const better = coverage > baseline.coverage ||
                (coverage === baseline.coverage && (baseline.supplier === null || total < baseline.total));
            if (better)
                baseline = { supplier: s, total, coverage };
        }
        ...
        const recommendedPlan = canConsolidate && baselineNet < splitNet ? 'consolidate' : 'split';
        // --- Materialize the chosen plan ---
        const grouped = {};
        let chosenTotal = 0;
        const chosenSuppliers = new Set();
        let maxDays = 0;
        const resultItems = analyses.map((a) => {
            if (!a.best) {
                return {
                    query: a.query,
                    supplier: null,
        ...
        const savingsFraction = Number.isFinite(otherNet) && otherNet > 0 ? estimatedSavings / otherNet : 0;
    ...
    static async optimize(userId, req) {
        const category = req.category;
        if (!category)
            throw new http_1.ApiError(400, 'Category is required');
        const valid = data_1.CATEGORY_SUPPLIERS[category];
        if (!valid)
            throw new http_1.ApiError(400, `Unknown category: ${category}`);
        const items = (req.items || []).filter((i) => i.query && i.query.trim());
        if (!items.length)
            throw new http_1.ApiError(400, 'At least one item is required');
        let suppliers = (req.suppliers && req.suppliers.length ? req.suppliers : valid).filter((s) => valid.includes(s));
        if (!suppliers.length)
            suppliers = valid;
        ...
        const gathered = await Promise.all(items.map(async (it) => ({
            query: it.query.trim(),
            quantity: Math.max(1, Math.round(it.quantity || 1)),
            products: await SearchService_1.SearchService.gather(it.query.trim(), category, suppliers),
        })));
        ...
        if (fulfilledItems.length > 0) {
            BasketHistoryRepository_1.basketHistoryRepository
                .create({
                userId,
                category,
                suppliers,
                ...
                .catch((e) => logger_1.logger.error('Failed to persist basket history', e));
        }
        return { category, weightProfile, ...plan };
    }
}
exports.BasketOptimizationService = BasketOptimizationService;
